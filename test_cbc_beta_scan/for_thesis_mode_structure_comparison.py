...
def make_comparison_beta004_with_bugfix(diff=False, for_talk=False):
    """ """
    marker_size = 12
    legend_fontsize = 16
    marker_list = ["s", "o", "P", "X", "v", "^", "<", ">", "1", "2", "3"]
    ...
    outnc_longnames= [
        beta_04_for_thesis_folder + "gs2_nt64_np4.out.nc",
        beta_04_for_thesis_folder + "stella_implicit_np4_nt64.out.nc",
        beta_04_for_thesis_folder + "stella_implicit_np4_nt64_with_bugfix.out.nc",
        #beta_04_for_thesis_folder + "stella_explicit_zupw0_src_h_fapar1_fbpar1.out.nc",
        #beta_04_for_thesis_folder + "stella_implicit_np4_nt64_tupw0.out.nc",
        ]
    if for_talk:
        labels =[
            r"GS2 ($n_z=64$, $n_{period}=4$)",
            r"stella ($n_z=64$, $n_{period}=4$)",
            r"stella ($n_z=64$, $n_{period}=4$) with bugfix",
            #r"stella (fully explicit) ($n_z=64$, $n_{p}=4$)",
            #r"stella (implicit) ($n_z=64$, $n_{period}=4$, $u_t=0$)",
        ]
    else:
        labels =[
                 r"GS2 ($n_z=64$, $n_{period}=4$)",
                  r"stella (implicit) ($n_z=64$, $n_{period}=4$)",
                  r"stella (implicit, bugfix) ($n_z=64$, $n_{period}=4$)",
                  # r"stella (explicit) ($n_z=64$, $n_{period}=4$)",
                  #r"stella (implicit) ($n_z=64$, $n_{period}=4$, $u_t=0$)",
                    ]
    col_list = [default_cmap(1), default_cmap(3), default_cmap(4),
    ...
    if diff:
        if for_talk:
            plt.savefig("for_talk_beta_004_mode_comparison_diff_bugfix.eps")
        else:
            plt.savefig("beta_004_mode_comparison_diff_bugfix.eps")
    else:
        if for_talk:
            plt.savefig("for_talk_beta_004_mode_comparison_fields_bugfix.eps")
        else:
            plt.savefig("beta_004_mode_comparison_fields_bugfix.eps")
    return
...
if __name__ == "__main__":
    ...
    make_comparison_beta004_with_bugfix(diff=False)
    make_comparison_beta004_with_bugfix(diff=True)
